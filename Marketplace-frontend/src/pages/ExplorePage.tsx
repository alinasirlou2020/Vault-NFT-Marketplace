...
export function ExplorePage() {
  ...
  const maxPage = Math.max(0, Math.ceil(totalListings / Number(ITEMS_PER_PAGE)) - 1)

  // Client-side filtering/sorting of the current page data
  const filteredListings = listings?.filter(l => {
    if (!search) return true
    const term = search.toLowerCase()
    return l.nftAddress.toLowerCase().includes(term) || l.tokenId.toString().includes(term)
  }).sort((a, b) => {
  ...
  return (
    <div className="container mx-auto px-4 md:px-6 pt-32 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6">
        <div>
          <h1 className="text-4xl font-bold text-white mb-2 tracking-tight">Explore NFTs</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          ...
          <div className="relative group">
            ...
            >
              <option value="newest" className="bg-black">Recently Listed</option>
        ...
        {isLoading ? (
          ...
          <div className="col-span-full flex flex-col items-center justify-center py-20 text-center glass-strong rounded-2xl border border-white/5">
            ...
            <p className="text-white/50 max-w-sm">
              We couldn't find any NFTs matching your current filters. Try adjusting your search or come back later.
            </p>
          </div>
        )}
      </div>
