...
// Whole non-negative integers only (no decimals, no scientific notation, no leading '-').
const TOKEN_ID_PATTERN = /^\d+$/;

// Strictly positive decimal numbers (e.g. "0.5", "12", "12.001") — rejects "0", "-1", "1e5", etc.
const POSITIVE_DECIMAL_PATTERN = /^\d*\.?\d+$/;

export function ListPage() {
  // All hooks must run unconditionally, on every render, in the same order —
  // so nothing here can sit behind an early `return` based on isConnected.
  ...
  const validateBase = () => {
    ...
    if (!tokenId || !TOKEN_ID_PATTERN.test(tokenId)) {
      toast.error("Token ID must be a whole number");
      return false;
    }
    return true;
  };

  const validatePositiveAmount = (value: string, label: string) => {
    if (!value || !POSITIVE_DECIMAL_PATTERN.test(value) || Number(value) <= 0) {
      toast.error(`Enter a valid ${label} greater than 0`);
      return false;
    }
    return true;
  };
  ...
  const handleListFixed = async () => {
    if (!validateBase() || !validatePositiveAmount(price, "price")) return;
    try {
  ...
  const handleCreateAuction = async () => {
    if (
      !validateBase() ||
      !validatePositiveAmount(startingPrice, "starting price")
    )
      return;
    try {
  ...
  const handleListRent = async () => {
    if (
      !validateBase() ||
      !validatePositiveAmount(pricePerDay, "price per day")
    )
      return;
    try {
      const tx = await listForRent(
        nftAddress as `0x${string}`,
        BigInt(tokenId),
        pricePerDay,
      );
    ...
    <div className="container mx-auto px-4 md:px-6 pt-32 pb-20 max-w-3xl">
      <div className="text-center mb-12">
        ...
        <p className="text-white/50">
          Choose how you want to offer your NFT to the market.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
        ...
        >
          ...
          <Tag
            className={cn(
              "w-8 h-8 mb-4",
              mode === "fixed" ? "text-neon-purple" : "text-white/50",
            )}
          />
          <h3
            className={cn(
              "text-xl font-bold mb-2",
              mode === "fixed" ? "text-white" : "text-white/70",
            )}
          >
            Fixed Price
          </h3>
          <p className="text-sm text-white/50">
            List your item at a set price for immediate purchase.
          </p>
        </button>
        ...
        >
          {mode === "auction" && (
            <div className="absolute inset-0 bg-gradient-to-br from-neon-pink/20 to-transparent pointer-events-none" />
          )}
          <Gavel
            className={cn(
              "w-8 h-8 mb-4",
              mode === "auction" ? "text-neon-pink" : "text-white/50",
            )}
          />
          <h3
            className={cn(
              "text-xl font-bold mb-2",
              mode === "auction" ? "text-white" : "text-white/70",
            )}
          >
            Auction
          </h3>
        ...
        >
          {mode === "rent" && (
            <div className="absolute inset-0 bg-gradient-to-br from-neon-cyan/20 to-transparent pointer-events-none" />
          )}
          <Key
            className={cn(
              "w-8 h-8 mb-4",
              mode === "rent" ? "text-neon-cyan" : "text-white/50",
            )}
          />
          <h3
            className={cn(
              "text-xl font-bold mb-2",
              mode === "rent" ? "text-white" : "text-white/70",
            )}
          >
            Rental
          </h3>
          <p className="text-sm text-white/50">
            Earn passive income by renting out utility.
          </p>
        </button>
      </div>
      ...
      <div className="glass-strong border border-white/10 rounded-3xl p-6 md:p-10 relative">
        <div className="flex items-start gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl mb-8">
          ...
          <div className="text-sm text-yellow-200/90 leading-relaxed">
            <strong>Important:</strong> You must approve the marketplace
            contract to manage your NFT before listing. This interface assumes
            approval is done. If transactions fail, ensure you've called{" "}
            <code className="bg-black/30 px-1 py-0.5 rounded">
              setApprovalForAll
            </code>{" "}
            on your NFT contract first.
          </div>
        </div>
