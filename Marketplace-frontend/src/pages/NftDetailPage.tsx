...
import { ExternalLink, Copy, Tag, Gavel, Key, ShieldCheck, Clock, CheckCircle2, Loader2, RefreshCw, Hexagon } from 'lucide-react'
import { motion, AnimatePresence } from 'framer-motion'

import { 
  useListing, useAuction, useOffer, useRental,
  useCancelListing, useCancelAuction, useEndAuction, useCancelOffer, useAcceptOffer,
  useCancelRental, useEndRental
...
import { getAddressUrl, getTokenUrl, getTxUrl } from '@/config/contracts'
...
export function NftDetailPage() {
  ...
  const nftAddress = params.nftAddress as `0x${string}`
  const tokenId = BigInt(params.tokenId || '0')
  ...
  // Reads
  const { metadata, isLoading: metaLoading } = useNftMetadata(nftAddress, tokenId)
  const { data: collectionName } = useCollectionName(nftAddress)
  const { data: listing, refetch: refetchListing } = useListing(nftAddress, tokenId)
  const { data: auction, refetch: refetchAuction } = useAuction(nftAddress, tokenId)
  const { data: offer, refetch: refetchOffer } = useOffer(nftAddress, tokenId)
  const { data: rental, refetch: refetchRental } = useRental(nftAddress, tokenId)
  ...
  const handleAction = async (actionFn: () => Promise<string | undefined>, successMsg: string) => {
    try {
      ...
      if (tx) {
        toast.success(
          <div className="flex flex-col gap-1">
            ...
            <a href={getTxUrl(tx)} target="_blank" rel="noreferrer" className="text-xs underline text-white/70 hover:text-white">View transaction</a>
          </div>
        )
        refreshAll()
      }
  ...
  const isAuctionEnded = auction?.active && auction.endTime <= Math.floor(Date.now() / 1000)
  ...
  const isOwner = currentOwner && connectedWallet && currentOwner.toLowerCase() === connectedWallet.toLowerCase()
    ...
    <div className="container mx-auto px-4 md:px-6 pt-24 pb-20">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="rounded-2xl overflow-hidden glass border border-white/10 shadow-2xl relative bg-black/40">
            ...
            <div className="absolute top-4 left-4 flex flex-col gap-2">
              {isListingActive && <div className="badge-listed px-3 py-1 rounded-md text-xs font-bold shadow-lg backdrop-blur-md border">For Sale</div>}
              {isAuctionActive && !isAuctionEnded && <div className="badge-auction px-3 py-1 rounded-md text-xs font-bold shadow-lg backdrop-blur-md border">Live Auction</div>}
              {isRentalListed && !isRented && <div className="badge-rental px-3 py-1 rounded-md text-xs font-bold shadow-lg backdrop-blur-md border">For Rent</div>}
              {isRented && <div className="bg-white/10 border border-white/20 text-white/90 px-3 py-1 rounded-md text-xs font-bold shadow-lg backdrop-blur-md">Rented</div>}
            </div>
          </div>
              ...
              <div className="p-4 grid grid-cols-2 gap-3">
                {metadata.attributes.map((attr, i) => (
                  <div key={i} className="bg-black/40 border border-white/5 rounded-lg p-3 text-center">
                    <div className="text-xs text-neon-purple mb-1 font-medium uppercase tracking-wider">{attr.trait_type}</div>
        ...
        <div className="lg:col-span-7 space-y-6 pt-4 lg:pt-0">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Link href={`/explore`} className="text-neon-cyan hover:underline text-sm font-medium">
                {collectionName || 'Nexus Collection'}
              </Link>
              <div className="flex gap-2">
                <button onClick={() => copyToClipboard(nftAddress)} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-white/50 hover:text-white transition-colors">
                  <Copy className="w-4 h-4" />
                </button>
                <a href={getTokenUrl(nftAddress, tokenId.toString())} target="_blank" rel="noreferrer" className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-white/50 hover:text-white transition-colors">
            ...
            <div className="flex flex-wrap items-center gap-6 pt-2">
              <div className="flex flex-col">
                <span className="text-xs text-white/40 mb-1 font-medium">Contract Address</span>
                <span className="text-sm font-mono text-white/80">{shortenAddress(nftAddress, 6)}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-xs text-white/40 mb-1 font-medium">Token ID</span>
                <span className="text-sm font-mono text-white/80">{tokenId.toString()}</span>
              </div>
              {currentOwner && (
                <div className="flex flex-col">
                  <span className="text-xs text-white/40 mb-1 font-medium">Owner / Seller</span>
                  <Link href={`/profile/${currentOwner}`} className="text-sm font-mono text-neon-purple hover:underline flex items-center gap-1">
                    {shortenAddress(currentOwner, 6)}
                    {isOwner && <span className="bg-neon-purple/20 text-neon-purple text-[10px] px-1.5 py-0.5 rounded ml-1">You</span>}
                  </Link>
                </div>
              )}
            </div>
          ...
          <div className="space-y-4">
            ...
            {isListingActive && listing && (
              <div className="glass-strong border border-neon-purple/30 rounded-2xl p-6 relative overflow-hidden group">
                ...
                <div className="text-sm text-white/50 mb-2 font-medium uppercase tracking-wider">Fixed Price</div>
                <div className="text-4xl font-bold text-white mb-6">
                  {formatMatic(listing.price)} <span className="text-xl text-white/50 font-normal">MATIC</span>
                </div>
                  ...
                  {!isOwner ? (
                    <>
                      ...
                      <button 
                        ...
                        className="flex-1 py-4 px-6 rounded-xl font-bold bg-white/5 border border-white/10 text-white hover:bg-white/10 transition-all"
                      >
                        Make Offer
                      </button>
                    </>
                    ...
                    <button 
                      onClick={() => handleAction(() => cancelListing(nftAddress, tokenId), 'Listing cancelled')}
                    ...
                    >
                      {cancelListingPending ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Cancel Listing'}
                    </button>
                  )}
                </div>
              </div>
                ...
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <div className="text-sm text-white/50 mb-2 font-medium uppercase tracking-wider">
                      {auction.highestBid > 0n ? 'Current Highest Bid' : 'Starting Price'}
                    </div>
                    <div className="text-4xl font-bold text-white">
                      {formatMatic(auction.highestBid > 0n ? auction.highestBid : auction.startingPrice)} <span className="text-xl text-white/50 font-normal">MATIC</span>
                    </div>
                    {auction.highestBid > 0n && (
                      <div className="mt-1 text-sm text-white/50 flex items-center gap-1">
                        by <Link href={`/profile/${auction.highestBidder}`} className="text-neon-pink hover:underline font-mono">{shortenAddress(auction.highestBidder)}</Link>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="text-right">
                    <div className="text-sm text-white/50 mb-2 font-medium uppercase tracking-wider flex items-center justify-end gap-1">
                      <Clock className="w-4 h-4" /> {isAuctionEnded ? 'Status' : 'Ends In'}
                    </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {!isOwner ? (
                    ...
                    <>
                      {isAuctionEnded ? (
                        <button 
                          onClick={() => handleAction(() => endAuction(nftAddress, tokenId), 'Auction ended successfully')}
                        ...
                        >
                          {endAuctionPending ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Settle Auction'}
                        </button>
                      ) : auction.highestBid === 0n ? (
                        <button 
                          onClick={() => handleAction(() => cancelAuction(nftAddress, tokenId), 'Auction cancelled')}
                        ...
                        >
                          {cancelAuctionPending ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Cancel Auction'}
                        </button>
                      ) : (
            ...
            {offer?.active && (
              <div className="glass-strong border border-neon-blue/30 rounded-2xl p-5 bg-[#0d1520]">
                ...
                <div className="flex justify-between items-center bg-black/40 p-4 rounded-xl border border-white/5">
                  <div>
                    <div className="text-2xl font-bold text-white">{formatMatic(offer.amount)} MATIC</div>
                    <div className="text-sm text-white/50 mt-1">
                      From: <Link href={`/profile/${offer.buyer}`} className="text-neon-blue hover:underline font-mono">{shortenAddress(offer.buyer)}</Link>
                    </div>
                  </div>
                  ...
                  <div>
                    {isOwner ? (
                      <button 
                        onClick={() => handleAction(() => acceptOffer(nftAddress, tokenId), 'Offer accepted')}
                      ...
                      >
                        {acceptOfferPending ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Accept'}
                      </button>
                    ) : connectedWallet && offer.buyer.toLowerCase() === connectedWallet.toLowerCase() ? (
                      <button 
                        onClick={() => handleAction(() => cancelOffer(nftAddress, tokenId), 'Offer cancelled')}
                      ...
                      >
                        {cancelOfferPending ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Cancel'}
                      </button>
                    ) : null}
                  </div>
                </div>
            ...
            {isRentalListed && rental && (
              <div className="glass-strong border border-neon-cyan/30 rounded-2xl p-6 relative overflow-hidden group">
                ...
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <div className="text-sm text-white/50 mb-2 font-medium uppercase tracking-wider">Rental Rate</div>
                    <div className="text-4xl font-bold text-white">
                      {formatMatic(rental.pricePerDay)} <span className="text-xl text-white/50 font-normal">MATIC/day</span>
                    </div>
                  </div>
                  
                  {isRented && (
                    <div className="text-right">
                      <div className="text-sm text-white/50 mb-2 font-medium uppercase tracking-wider">Rented Until</div>
                      <div className="text-xl font-bold bg-black/40 px-3 py-1.5 rounded-lg border border-white/5">
                ...
                <div className="flex flex-wrap gap-3">
                  {!isRented ? (
                    ...
                    !isOwner ? (
                      ...
                      <button 
                        onClick={() => handleAction(() => cancelRental(nftAddress, tokenId), 'Rental listing cancelled')}
                        disabled={cancelRentalPending}
                        className="w-full py-4 px-6 rounded-xl font-bold bg-red-500/10 border border-red-500/30 text-red-400 hover:bg-red-500/20 transition-all flex justify-center items-center gap-2"
                      >
                        {cancelRentalPending ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Cancel Rental Listing'}
                      </button>
                    )
                  ) : (
                    // Currently rented
                    <div className="w-full">
                      <div className="flex items-center gap-2 p-3 bg-white/5 rounded-lg border border-white/10 mb-4">
                        ...
                        <span className="text-sm text-white/80">
                          Rented to <Link href={`/profile/${rental.tenant}`} className="font-mono text-neon-cyan hover:underline">{shortenAddress(rental.tenant)}</Link>
                        </span>
                      </div>
                      
                      {(isOwner || (connectedWallet && rental.tenant.toLowerCase() === connectedWallet.toLowerCase())) && (
                        <button 
                          onClick={() => handleAction(() => endRental(nftAddress, tokenId), 'Rental ended')}
                        ...
                        >
                          {endRentalPending ? <Loader2 className="w-5 h-5 animate-spin" /> : 'End Rental'}
                        </button>
                      )}
                    </div>
                  )}
            ...
            {!isListingActive && !isAuctionActive && !isRentalListed && (
              <div className="glass-strong border border-white/5 rounded-2xl p-8 text-center bg-white/5">
                ...
                <h3 className="text-xl font-bold text-white mb-2">Not Listed</h3>
                <p className="text-white/50 max-w-sm mx-auto mb-6">
                  This asset is currently not listed for sale, auction, or rent on NexusMarket.
                </p>
                {isOwner && (
                  <Link 
                    href="/list"
            ...
            {!isOwner && !isListingActive && !isAuctionActive && (
               <button 
                onClick={() => setOfferOpen(true)}
      ...
      {auction && (
        <BidModal 
          ...
          nftAddress={nftAddress} 
          tokenId={tokenId} 
          currentPrice={auction.highestBid > 0n ? auction.highestBid : auction.startingPrice}
